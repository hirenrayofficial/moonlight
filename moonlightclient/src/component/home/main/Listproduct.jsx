  ...
  return (
    <section className="pl-root">
      <style>{`
        ...
        @media (max-width: 720px) { .pl-root { padding: 40px 20px 48px; } }

        .pl-mono { font-family: 'IBM Plex Mono', monospace; letter-spacing: 0.03em; }
        ...
        .pl-tab:hover { color: var(--ink); border-color: var(--ink); }
        .pl-tab.active { background: var(--ink); color: var(--bg); border-color: var(--ink); }
        .pl-tab:focus-visible { outline: 2px solid var(--ink); outline-offset: 2px; }
        ...
        .pl-image-wrap { position: relative; }
        .pl-image {
          width: 100%;
          aspect-ratio: 1 / 1;
          object-fit: cover;
        ...
        .pl-tag.low { background: var(--warn); color: #fff; }
        ...
        .pl-card:hover .pl-quick-add,
        .pl-quick-add:focus-visible { transform: translateY(0); }
        ...
        .pl-view-all:hover { border-color: var(--ink); }
        .pl-view-all:focus-visible { outline: 2px solid var(--ink); outline-offset: 2px; }
      `}</style>
      ...
      <div className="pl-inner">
        <div className="pl-head">
          ...
          <div className="pl-tabs" role="tablist" aria-label="Filter by category">
            {CATEGORIES.map((c) => (
              <button
                key={c}
                role="tab"
            ...
            {visible.map((p) => (
              <div className="pl-card" key={p.name}>
                <div className="pl-image-wrap">
                  {p.tag && (
                    <span className={`pl-tag ${p.tag === "Low stock" ? "low" : ""}`}>{p.tag}</span>
                  )}
